...
namespace ORB_SLAM3
{
    ...
    LocalMapping::LocalMapping(System *pSys, Atlas *pAtlas, const float bMonocular, bool bInertial, const string &_strSeqName) : mpSystem(pSys), mbMonocular(bMonocular), mbInertial(bInertial), mbResetRequested(false), mbResetRequestedActiveMap(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas), bInitializing(false),
                                                                                                                                 ...
                                                                                                                                 mIdxInit(0), mScale(1.0), mInitSect(0), mbNotBA1(true), mbNotBA2(true), mIdxIteration(0), infoInertial(Eigen::MatrixXd::Zero(9, 9)), mbMergedFlag(false), mbImuInitFlag(false)
    {
        mnMatchesInliers = 0;
    ...
    void LocalMapping::Run()
    {
        ...
        while (1)
        {
            ...
            if (CheckNewKeyFrames() && !mbBadImu)
            {
                ...
                // -------------------------------------------------
                // TODO: 如果说关闭了回环线程，那么此处还需要进行修改（只要IMU完成了初始化即可）
                /*
                if (!mpSystem->mbSystemCtlFlag) // 控制标志位,增加额外的判断机制
                {
                    if (!mpSystem->mbLocalization) // Create map mode.
                    {
                        if (mbImuInitFlag)
                        {
                            // 对于建图模式来说此时系统已经完成了初始化.
                            mpSystem->mbSystemInitFlag = true;
                            mpSystem->mbSystemCtlFlag = true;
                            std::cout << "<---- 建图模式下系统初始化成功 ---->" << std::endl;
                        }
                    }
                    else // location mode.
                    {
                        // TODO: 如果关闭了回环检测的线程请取消注释以下代码并取消后面代码的注释.
                        if (mbImuInitFlag && mbMergedFlag)
                        {
                            // 对于定位模式来说此时完成了定位的回环，系统完成了初始化.
                            mpSystem->mbSystemInitFlag = true;
                            mpSystem->mbSystemCtlFlag = true;
                        }
                    }
                ...
                */
                ...
                // TODO: 如果关闭了回环检测线程那么只需要保证IMU完成了初始化即可.
                if (!mpSystem->mbSystemCtlFlag)
                {
                    if (mbImuInitFlag)
                    {
                        mpSystem->mbSystemInitFlag = true;
    ...
    void LocalMapping::ProcessNewKeyFrame()
    {
        ...
        // Compute Bags of Words structures（计算当前帧的词袋模型）
        mpCurrentKeyFrame->ComputeBoW();

        // Associate MapPoints to the new keyframe and update normal and descriptor（将 MapPoints 关联到新的关键帧并更新法线和描述符）
