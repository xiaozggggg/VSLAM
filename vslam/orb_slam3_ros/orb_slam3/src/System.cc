...
namespace ORB_SLAM3
{
    ...
    System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
                   const bool bUseViewer, const int initFr, const string &strSequence) : mSensor(sensor), mpViewer(static_cast<Viewer *>(NULL)), mbReset(false), mbResetActiveMap(false),
                                                                                         ...
                                                                                         mbLocalization(false),mbSystemCtlFlag(false),
                                                                                         mbSystemInitFlag(false)
    {
        ...
        // Shang:Working mode switch.
        node = fsSettings["System.WorkingMode"];
        if(!node.empty())
        {
            if(node.real() != 0)
                mbLocalization = true;
        ...
        // Mr.ShiKai:add_new.
        node = fsSettings["Yolo.enable"];
        if (!node.empty() && node.isString())
        {
            yolo_enable = (string)node;
            Yolomsg_enable = yolo_enable;
            if(yolo_enable=="true")
            {
                cout << "Yolo功能已打开" << endl;
            }
            else if(yolo_enable=="false")
            {
                cout << "Yolo功能已关闭" << endl;
            }
            else
            {
                cout << "Warning： 请在yaml文件中正确配置yolo.enable，输入true或者false" << endl;
            }
        }
        else
        {
            cout << "请检查yolo使能开关是否配置正确" << endl;
        }

        node = fsSettings["Yolo.coco"];
        if (!node.empty() && node.isString())
        {
            coco_file = (string)node;
        }
        
        node = fsSettings["Yolo.engine"];
        if (!node.empty() && node.isString())
        {
            engine_file = (string)node;
        }
        ...
        if (mStrLoadAtlasFromFile.empty())
        {
            ...
            // Mr.Chen Changed: 2023.4.24,此处增加一个判断，用于同时支持.txt文件和binary文件
            bool bVocLoad;
            std::string::size_type index = strVocFile.find(".txt");
        ...
        if (yolo_enable=="true")
        {
            mpDetector = new YOLOv5_seg();
            mpTracker->SetDetector(mpDetector);
        }
        
        // mpTracker->SetDetector();
    ...
    Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename)
    {
        ...
        // Check mode change
        {
            // 开启纯定位模式判断:如果允许开启定位模式，那么就开启（允许开启的前提是视觉或者视觉+IMU部分已经成功初始化）
            //if(mpTracker->allow_start)
            //{
   ...
   //yolov5
    void System::GetYoloData()
    {
        if (!(mpTracker->mmDetectMap.empty()))
        {
            for (auto vit = mpTracker->mmDetectMap.begin(); vit != mpTracker->mmDetectMap.end(); vit++)
            {
                for (auto area : vit->second)
                {
                    Yolomsg.push_back(vit->first);
                    Yolomsg.push_back(to_string(area.RecArea.x));
                    Yolomsg.push_back(to_string(area.RecArea.y));
                    Yolomsg.push_back(to_string(area.RecArea.width));
                    Yolomsg.push_back(to_string(area.RecArea.height));
                }

            }
        }
        mpTracker->mpDetector->mvDynamicArea.clear();
        mpTracker->mpDetector->mmDetectMap.clear();
