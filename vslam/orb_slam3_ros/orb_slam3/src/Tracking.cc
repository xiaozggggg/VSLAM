...
void Tracking::SetDetector(YOLOv5_seg* pDetector)
{
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if(yolo_enable=="true")
    {
        cv::Mat img_L;
        cv::Mat img_R;
        img_L= imRectLeft.clone();
        img_R= imRectRight.clone();
        
        mpDetector->detect(img_L,img_R);
        mpORBextractorLeft->mvDynamicArea = mpDetector->mvDynamicArea;
        mmDetectMap = mpDetector->mmDetectMap;
    }
    else
    {
    ...
    // Mr.Chen: image cut:2023-8-28 PM
    // mImGray = mImGray(cv::Range(80,400),cv::Range(0,848));
    // imGrayRight = imGrayRight(cv::Range(80,400),cv::Range(0,848));

    // Mr.Chen,Tips: Current mode used function is third.
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        else
        {
            ...
            // 增加了mState==RECENTLY_LOST代码，用于解决定位模式下状态不改变的问题，另外一种修复方案是：if(mState==RECENTLY_LOST && !bOK && mCurrentFrame.mTimeStamp-mTimeStampLost>time_recently_lost_visual)
        ...
        if(mState==LOST)
        {
            ...
            if(mpSystem->mbSystemCtlFlag) // mbSystemCtlFlag为真实际上从侧面反应当前系统已经完成了初始化.
            {
                // TODO:这里是否需要区分建图模式和定位模式？
                // 如果是建图模式那么只需要重置对应的标志位，如果是建图模式是否还需要再次载入外部地图呢？
                if(!mpSystem->mbLocalization)
                {
                    mpSystem->mbSystemCtlFlag = false;
                    mpSystem->mbSystemInitFlag = false;
                }
                else
                {
                    mpSystem->mbSystemCtlFlag = false;
                    mpSystem->mbSystemInitFlag = false;
                    mpLocalMapper->mbMergedFlag = false;
                }
            }

            /*
            if(mpSystem->mbLocalization && mpSystem->mbSystemCtlFlag && mpSystem->mbSystemInitFlag)
            {
                // 重置地图后再载入地图
                mpSystem->ResetActiveMap();
                bool isRead = mpSystem->LoadAtlas(1);
                if(isRead)
                {
                    mpSystem->mbSystemCtlFlag = false;
                    mpSystem->mbSystemInitFlag = false;
...
// 该函数可能又改进的空间,修改匹配时候的约束因子，当前默认是0.7
